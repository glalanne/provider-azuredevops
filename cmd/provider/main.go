...
import (
	...
	clusterapis "github.com/glalanne/provider-azuredevops/apis/cluster"
	namespacedapis "github.com/glalanne/provider-azuredevops/apis/namespaced"

	apiextensionsv1 "k8s.io/apiextensions-apiserver/pkg/apis/apiextensions/v1"

	clustercontroller "github.com/glalanne/provider-azuredevops/internal/controller/cluster"
...
func main() {
	...
	logr := logging.NewLogrLogger(zl.WithName("provider-azuredevops"))
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped azuredevops controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced azuredevops controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped azuredevops controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup(mgr, namespacedOpts), "Cannot setup namespaced azuredevops controllers")
	}
