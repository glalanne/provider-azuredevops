...
import (
	...
	clusterv1beta1 "github.com/glalanne/provider-azuredevops/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/glalanne/provider-azuredevops/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(tfProvider *schema.Provider) terraform.SetupFn { //nolint:gocyclo
	return func(ctx context.Context, client client.Client, mg xpresource.Managed) (terraform.Setup, error) {
		...
		return ps, errors.Wrap(configureNoForkDBXClient(ctx, &ps, *tfProvider), "failed to configure the no-fork azuredevops client")
	}
}
...
func defaultAuth(ctx context.Context, pcSpec *namespacedv1beta1.ProviderConfigSpec, ps *terraform.Setup, client client.Client) error { // nolint: gocyclo
	...
	if v, ok := creds[keyUseMsi]; ok {
		ps.Configuration[keyUseMsi] = v
	}
	if v, ok := creds[keyUseOIDC]; ok {
		ps.Configuration[keyUseOIDC] = v
	}
	...
	if v, ok := creds[keyPAT]; ok {
		ps.Configuration[keyPAT] = v
	}
	if v, ok := creds[keyOIDCToken]; ok {
		ps.Configuration[keyOIDCToken] = v
	}
	if v, ok := creds[keyUseOIDC]; ok {
		ps.Configuration[keyUseOIDC] = v
	}
