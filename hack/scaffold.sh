...
resources=$(jq --raw-output '.provider_schemas."registry.terraform.io/microsoft/azuredevops".resource_schemas | keys | .[]' ./config/schema.json)
root_folder="./config"
...
do
  resource_name=$(echo $resource | sed 's/azuredevops//')
